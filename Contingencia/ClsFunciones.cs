...
namespace Contingencia
{
    public class ClsFunciones
    {
        ...
        public static int LocalizarClaveCombo(ComboBox combo, string psValor)
        {
            int contador = 0;
            int posicion = -1;
            string clave = Variables.Nulo;
            ...
            {
                for (contador = 0; contador <= combo.Items.Count - 1; contador++)
                {
                    clave = ObtenerClaveCadena(combo.Items[contador].ToString().Trim(), Variables.CorcheteAbre, Variables.CorcheteCierra);
                    if (clave == psValor.Trim())
                    {
                        posicion = contador;
                        break;
                    }
